...
# INPUTS
X = tf.placeholder(tf.int32, [None, None], 'X') # [batch_size, max_time_o]
X_len = tf.placeholder(tf.int32, [None], 'X_len') # [batch_size]
Y = tf.placeholder(tf.int32, [None, None], 'Y') # [batch_size, max_time_s]
Y_len = tf.placeholder(tf.int32, [None], 'Y_len') # [batch_size]
Y_targets = tf.placeholder(tf.int32, [None, None], 'Y_targets') # [batch_size, max_time_s]
...
masks = tf.sequence_mask(lengths=Y_len, 
                         ...
                         dtype=tf.float32)
loss = tf.contrib.seq2seq.sequence_loss(logits=dec_logits, # pred_logits
                                        targets=Y_targets, 
                                        weights=masks)
